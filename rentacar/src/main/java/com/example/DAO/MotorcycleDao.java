...
public interface MotorcycleDao extends JpaRepository<Motorcycle, Integer> {
    ...
    @Query("SELECT " + INFO_DTO_CONSTRUCTOR + " FROM Motorcycle m JOIN m.pricing p WHERE m.plateOrTailNumber = :identifier")
   ...
   @Query("SELECT " + LIST_DTO_CONSTRUCTOR
    ...
    @Query("SELECT " + LIST_DTO_CONSTRUCTOR + " FROM Motorcycle m JOIN m.pricing p WHERE m.mobilityType = :mobilityType")    
