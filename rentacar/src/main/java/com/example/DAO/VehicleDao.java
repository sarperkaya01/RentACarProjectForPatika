...
public interface VehicleDao extends JpaRepository<Vehicle, Integer> {
    ...
    @Query("SELECT " + DTO_CONSTRUCTOR_QUERY + " FROM Vehicle v JOIN v.properties p")
    ...
    @Query("SELECT " + DTO_CONSTRUCTOR_QUERY + " FROM Vehicle v JOIN v.properties p WHERE v.modelYear = :modelYear")
    List<VehicleListDto> findByModelYearAsDto(@Param("modelYear") Integer modelYear);

    @Query("SELECT " + DTO_CONSTRUCTOR_QUERY + " FROM Vehicle v JOIN v.properties p WHERE v.modelName = :modelName")
    List<VehicleListDto> findByModelNameAsDto(@Param("modelName") String modelName);

    @Query("SELECT " + DTO_CONSTRUCTOR_QUERY + " FROM Vehicle v JOIN v.properties p WHERE v.brandName = :brandName")
    List<VehicleListDto> findByBrandNameAsDto(@Param("brandName") String brandName);

    @Query("SELECT " + DTO_CONSTRUCTOR_QUERY + " FROM Vehicle v JOIN v.properties p WHERE v.vehicleStatus = :vehicleStatus")
    List<VehicleListDto> findByVehicleStatusAsDto(@Param("vehicleStatus") VehicleStatus vehicleStatus);

    @Query("SELECT " + DTO_CONSTRUCTOR_QUERY + " FROM Vehicle v JOIN v.properties p WHERE v.vehicleValue BETWEEN :minValue AND :maxValue")
